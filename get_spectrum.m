...
if size(matches, 2) == 1
    ...
    if size(matches, 2) == 1
      ...
      error('get_spectrum:identError', ...
            'does not match extension of known spectrum files (spm, chn)');
    end
end
fclose(fd);
...
if 100*spectrum.majvers + spectrum.minvers ~= spectrum.endheader
    error('get_spectrum_spm:headerError', ...
          'Error reading header ... version mismatch')
end
% Caution ahead.
...
spectrum.acquire_start = 24 * 3600 * ...
                         datenum(year, month, day, hour, min, sec) + hund;
